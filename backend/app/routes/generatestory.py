...
async def generate_story_and_frames(
    ...
    try:
        ...
        return {
            ...
            "user_topic": user_topic,  # Add user_topic to match frontend expectations
...
async def generate_story_endpoint(request: GenerateStoryRequest):
    """
    Generate a complete YouTube Shorts story and video frames based on selected trending video.
    """
    try:
        ...
        return GenerateStoryResponse(success=True, data=result)
    except HTTPException as he:
        raise he
    except Exception as e:
        print(f"Error in generate_story_endpoint: {e}")
        return GenerateStoryResponse(success=False, error=f"Story generation failed: {str(e)}")

@router.get("/health")
async def health_check():
    return {"status": "healthy", "service": "story_generation"}
