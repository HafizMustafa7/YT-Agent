...
def _check_video_config():
    ...
    if not (settings.VIDEO_DEFAULT_USER_ID or "").strip():
        missing.append("VIDEO_DEFAULT_USER_ID")
    if missing:
        raise HTTPException(
            status_code=503,
...
@video_router.post("/projects", response_model=Dict[str, Any])
async def create_video_project(request: CreateVideoProjectRequest):
    """
    Create a video project and project_frames from story result.
    Returns project_id for the video gen dashboard.
    """
    _check_video_config()

    frames_payload = [f.model_dump() for f in request.frames]
    try:
        project_id = video_service.create_video_project(request.title, frames_payload)
        logger.info("Video project created: %s (%d frames)", project_id, len(frames_payload))
        return {
            "success": True,
